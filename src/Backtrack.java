public class Backtrack {
   ...
   * trying to fill in - Base cases: - If the position is at the end of the puzzle grid, check if
  ...
  /*
   * Checks if filling the @param num in at location @param row, @param col makes the puzzle invalid
   * (unsolvable)
   */
  ...
  /**
   * Uses backtracking-7 algorithm to solve a partially filled @param puzzle grid and attempts to
  ...
  public static boolean solveSudoku(Sudoku puz) {
    ...
    for (int r = 0; r < puzzle.length; r++) {
      for (int c = 0; c < puzzle[0].length; c++) {
        if (puzzle[r][c] == 0) {
          row = r;
          col = c;
          isFilled = false; // There are still unfilled values in puzzle
          break;
        }
      }
      if (!isFilled) break; // Unfilled values means we continue to backtracking
    }
    if (isFilled) return true; // fully filled puzzle --> solution found
    ...
    for (int num = 1; num <= puzzle.length; num++) {
      if (isValid(
          puzzle, row, col, num)) { // placing num at row,col is valid (will be further backtracked)
